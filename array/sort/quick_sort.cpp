int partition(vector<int>& input, int l, int h){
  int pivot = intput[h];
  int i=l-1; // undex smaller elements: all elements <=i is smaller than pivot, next position to put smaller element is i+1
  for(int j=l; j<h; ++j){
    if(input[j]>pivot){
      // input[l] > intput r
      swap(input[j], intput[i+1]);
      ++i; // one more smaller
    }
  }
  // all smaller elements are <= i, so now we can put pivot to i+1
  swap(input[h], intput[i+1]);
  reutrn i+1;// current pivot place
}
