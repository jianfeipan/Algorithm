...
public:
    vector<int> minInterval(vector<vector<int>>& intervals, vector<int>& queries) {
        ...
        unordered_map<int, int> minlenForQuery;
        
        int currentInterval = 0;
        for(int i = 0; i < sortedQueries.size(); ++i){
            const auto& query = sortedQueries[i];
            // for each query we push the intervals start on left of query [[[[[ *  ...
            // this is monoton!!! we don't need to push them again for next query, because it's bigger
            while(currentInterval < intervals.size() && intervals[currentInterval][0] <= query){
                if(intervals[currentInterval][1]>=query){
                    const auto len = intervals[currentInterval][1] - intervals[currentInterval][0] + 1;
                    minLens.push({len, intervals[currentInterval][1]});
                }
                ++currentInterval;
            }// this moment i still don't know which are the 

            while(!minLens.empty() && minLens.top()[1] < query){
                minLens.pop(); 
                // those intervals close before taget, for sure wont include taret and next query
            }
            minlenForQuery[query] = minLens.empty() ? -1 : minLens.top()[0];
        }

        vector<int> res(queries.size());
        for(int i=0; i<queries.size(); ++i){
            res[i] = minlenForQuery[queries[i]];
        }
