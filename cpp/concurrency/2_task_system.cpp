...
public:
    ...
    void push(Task&& task) {
        {
            lock_guard<mutex> g(mtx_);
            if (done_) throw runtime_error("Push to stopped queue.");
    ...
    void run(unsigned index) {
        while (true) {
            ...
            for (unsigned i = 0; i<capacity_; ++i) {
                if(qs_[i].try_pop(task)) break;
            }
            if (!task) qs_[index].pop(task); // all queue locked, fall back to wait to my queue
            if (!task) return;
            task();
        }
    }
public:
    ...
    void async(Task&& task) {
        auto cur = index_.fetch_add(1, memory_order_relaxed);
        // try to push to a free queue
        for (unsigned i = 0; i<capacity_ * K; ++i) {
            if (qs_[ (cur+i)%capacity_].try_push(move(task))) return;
        }
        // all queue busy, fallback to blocking on my queue.
        qs_[cur % capacity_].push(move(task));
    }
};
